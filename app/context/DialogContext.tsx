...
export const DialogContextProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    return (
        <DialogContext.Provider value={{ open, setLoading, close }}>
            <div id="conf_dialog" className={`fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 flex justify-center items-start pt-10 ${!dialogState.isOpen && 'hidden'}`} style={{zIndex: "9999"}}>
                <div className="bg-white min-w-[400px] max-w-[95%] w-fit min-h-[200px] h-fit rounded-lg flex flex-col justify-center items-start gap-2 p-5">
                    ...
                    <div className="conf-dialog-body text-lg w-full max-w-[100%] overflow-x-auto">{dialogState.body}</div>
